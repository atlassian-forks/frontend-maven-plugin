...
public final class InstallNodeAndYarnMojo extends AbstractFrontendMojo {
     ...
     * 'v0.10.18'
     */
    @Parameter(property = "nodeVersion", defaultValue = "", required = false, readonly = true)
    ...
    public void execute(FrontendPluginFactory factory) throws InstallationException, LifecycleExecutionException {
        ...
        if (null != server) {
            factory.getNodeInstaller(proxyConfig).setNodeDownloadRoot(this.nodeDownloadRoot)
                .setNodeVersion(nodeVersion).setPassword(server.getPassword())
            ...
            factory.getNodeInstaller(proxyConfig).setNodeDownloadRoot(this.nodeDownloadRoot)
                .setNodeVersion(nodeVersion).install();
            factory.getYarnInstaller(proxyConfig).setYarnDownloadRoot(this.yarnDownloadRoot)
